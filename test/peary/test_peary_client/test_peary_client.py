...
def test_peary_client_init_socket(monkeypatch, mock_socket):
...
def test_peary_client_context_manager_runs_statements(monkeypatch, mock_socket):
    monkeypatch.setattr("socket.socket", mock_socket)
...
def test_peary_client_context_manager_enter_proxy_class(monkeypatch, mock_socket):
    monkeypatch.setattr("socket.socket", mock_socket)
    with peary.PearyClient("", proxy_class=MockProxy) as client:
        assert isinstance(client, MockProxy)


def test_peary_client_context_manager_enter_socket_connects(monkeypatch, mock_socket):
    monkeypatch.setattr("socket.socket", mock_socket)
    with peary.PearyClient("", proxy_class=MockProxy) as client:
...
def test_peary_client_context_manager_enter_socket_address(monkeypatch, mock_socket):
    monkeypatch.setattr("socket.socket", mock_socket)
...
def test_peary_client_context_manager_exit_socket_closes(monkeypatch, mock_socket):
    monkeypatch.setattr("socket.socket", mock_socket)

    mock_socket = mock_socket(socket.AF_INET, socket.SOCK_STREAM)
    with peary.PearyClient("", proxy_class=MockProxy):
        pass
    assert mock_socket.is_connected is False


def test_peary_client_context_manager_exit_socket_shutdown(monkeypatch, mock_socket):
    monkeypatch.setattr("socket.socket", mock_socket)
