...
def _mock_peary_device_getter_method(monkeypatch, mock_socket):
    def __mock_peary_device_getter_method(method, name, value):
        ...
        func = getattr(PearyDevice(0, mock_socket("", ""), MockProtocol), method)
        return func(name)

    return __mock_peary_device_getter_method


def test_peary_device_init_index(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert PearyDevice(0, mock_socket("", ""), MockProtocol).index == 0
    assert PearyDevice(1, mock_socket("", ""), MockProtocol).index == 1


def test_peary_device_init_name(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert PearyDevice(0, mock_socket("", ""), MockProtocol).name == "device.name 0"
    assert PearyDevice(1, mock_socket("", ""), MockProtocol).name == "device.name 1"


def test_peary_device_repr(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert str(PearyDevice(0, mock_socket("", ""), MockProtocol)) == "device.name 0(0)"
    assert str(PearyDevice(1, mock_socket("", ""), MockProtocol)) == "device.name 1(1)"


def test_peary_device_power_on(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert (
        PearyDevice(0, mock_socket("", ""), MockProtocol).power_on()
        == b"device.power_on 0"
    )
    assert (
        PearyDevice(1, mock_socket("", ""), MockProtocol).power_on()
        == b"device.power_on 1"
    )


def test_peary_device_power_off(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert (
        PearyDevice(0, mock_socket("", ""), MockProtocol).power_off()
        == b"device.power_off 0"
    )
    assert (
        PearyDevice(1, mock_socket("", ""), MockProtocol).power_off()
        == b"device.power_off 1"
    )


def test_peary_device_reset(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert (
        PearyDevice(0, mock_socket("", ""), MockProtocol).reset() == b"device.reset 0"
    )
    assert (
        PearyDevice(1, mock_socket("", ""), MockProtocol).reset() == b"device.reset 1"
    )


def test_peary_device_configure(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert (
        PearyDevice(0, mock_socket("", ""), MockProtocol).configure()
        == b"device.configure 0"
    )
    assert (
        PearyDevice(1, mock_socket("", ""), MockProtocol).configure()
        == b"device.configure 1"
    )


def test_peary_device_daq_start(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert (
        PearyDevice(0, mock_socket("", ""), MockProtocol).daq_start()
        == b"device.daq_start 0"
    )
    assert (
        PearyDevice(1, mock_socket("", ""), MockProtocol).daq_start()
        == b"device.daq_start 1"
    )


def test_peary_device_daq_stop(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert (
        PearyDevice(0, mock_socket("", ""), MockProtocol).daq_stop()
        == b"device.daq_stop 0"
    )
    assert (
        PearyDevice(1, mock_socket("", ""), MockProtocol).daq_stop()
        == b"device.daq_stop 1"
    )


def test_peary_device_list_registers(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")

    monkeypatch.setattr(MockProtocol, "request", mock_request)
    assert PearyDevice(0, mock_socket("", ""), MockProtocol).list_registers() == [
        "device.list_registers",
        "0",
    ]
    assert PearyDevice(1, mock_socket("", ""), MockProtocol).list_registers() == [
        "device.list_registers",
        "1",
    ]
...
def test_peary_device_set_register(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")
    ...
    for index, name, value in zip([0, 1], ["alpha", "beta"], [0, 1]):
        assert (
            PearyDevice(index, mock_socket("", ""), MockProtocol)
...
def test_peary_device_set_memory(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")
...
def test_peary_device_set_current(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")
...
def test_peary_device_set_voltage(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")
...
def test_peary_device_switch_on(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")
...
def test_peary_device_switch_off(monkeypatch, mock_socket):
    def mock_request(_, *args):
        return " ".join(args).encode("utf-8")
